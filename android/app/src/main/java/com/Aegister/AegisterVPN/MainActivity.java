package com.Aegister.AegisterVPN;
...
public class MainActivity extends FlutterActivity {
    ...
    private boolean isConnected() {
        ...
        if (cm != null) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                android.net.NetworkInfo nInfo = cm.getActiveNetworkInfo();
    ...
    private void setStage(String stage) {
        if (vpnStageSink != null && attached) {
            String localizedStage = getLocalizedStage(stage.toUpperCase());
            vpnStageSink.success(localizedStage);
        }
    }

    private String getLocalizedStage(String stage) {
        switch (stage) {
            case "CONNECTED":
                return getString(R.string.vpn_connected);
            case "DISCONNECTED":
                return getString(R.string.vpn_disconnected);
            case "WAIT":
                return getString(R.string.vpn_wait_connection);
            case "AUTH":
                return getString(R.string.vpn_authenticating);
            case "RECONNECTING":
                return getString(R.string.vpn_reconnect);
            case "NONETWORK":
                return getString(R.string.vpn_no_connection);
            case "CONNECTING":
                return getString(R.string.vpn_connecting);
            case "PREPARE":
                return getString(R.string.vpn_prepare);
            case "DENIED":
                return getString(R.string.vpn_denied);
            default:
                return getString(R.string.unknown_state); // Handle any unexpected state
        }
    }
}
